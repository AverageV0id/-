...
while run:
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.MOUSEBUTTONDOWN and event.button == 1 and open_craft_menu:
            if discount_card_rect.collidepoint(event.pos) and pearl_count >= 1 and coin_fish_count >= 5 and not discount_card_crafted:
        ...
        if event.type == pg.MOUSEBUTTONDOWN and event.button == 1 and open_craft_menu:
            if ring_rect.collidepoint(event.pos) and pearl_count >= 10 and coin_fish_count >= 5 and not ring_crafted:
                ...
                shop_sfx.play()
        if event.type == pg.MOUSEBUTTONDOWN and event.button == 1 and open_craft_menu:
            if lucky_coin_ring_rect.collidepoint(event.pos) and pearl_count >= 50 and gold_fish_count >= 15 and not lucky_coin_ring_crafted:
            ...
            if ring_crafted:
                if not lucky_coin_fished:
                    ...
                    if random.randint(1, 10) <= 5 and not coin_fish_count > 150:
                        fish1.image = red_fish_1
                    elif random.randint(1, 10) > 5 and not clam_count > 50 or pearl_count > 50:
                        fish1.image = clam_1
                    else:
                        if not gold_fish_count > 15:
                            fish1.image = gold_fish
